...
const StudentWorkTabPopUp: React.FC<StudentWorkPopupProps> = ({ studentData, activityId, teacherId, onClose, onSave, onDelete }) => {
    ...
    const handleDelete = async () => {
        try{
            setIsDeleting(true);
            const response = await fetch(`${BASE_URL}/hasil_karya?id=${studentData.id_work}`, {
                method: 'DELETE',
            });
            ...
            const updatedStudentWork = { ...studentWork, status: StudentWorkStatus.NOT_YET, file: "", file_type: ""};
    ...
    return (
        <div style={{ position: "fixed", top: "0", left: "0", width: "100%", height: "100%", backgroundColor: "rgba(0, 0, 0, 0.5)", zIndex: "1000", textAlign:"center" }}>
            ...
            <h2 className="text-heading-2 text-2xl text-text-100 font-semibold mb-4">{studentWork.name}</h2>
                {studentWork.file ? (
                    <div className="popup-content-uploaded" style={{ textAlign: "center", padding: "30px" }}>
                        <div className="bg-neutral3 flex justify-center items-center rounded-lg" style={{ height: '300px' }}>
                            {['video/mp4', 'video/webm', 'video/ogg'].includes(studentWork.file_type) ? (
                                <video src={studentWork.file} controls />
                            ) : ['image/png', 'image/jpeg', 'image/gif', 'image/webp'].includes(studentWork.file_type) ? (
                                <img src={studentWork.file} alt={studentWork.work_name} style={{ width: '90%', height: '90%', objectFit: 'cover' }} />
                            ) : (
                                <div>
                                    ...
                                    <button className="bg-persian-blue-500 text-white py-2 px-4 rounded-md text-sm font-semibold hover:bg-persian-blue-600 transition duration-300">
                                        <a href={studentWork.file} download>Unduh</a>
                                    </button>
                                </div>
                            )}
                        </div>
                        <h2 className="text-lg font-semibold mt-4 overflow-hidden whitespace-nowrap text-overflow-ellipsis" style={{ maxWidth: "100%", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{studentWork.work_name}</h2>
                        <button onClick={handleDelete} disabled={isSaving} className="bg-red-500 text-white py-2 px-4 rounded-md text-sm font-semibold hover:bg-red-600 transition duration-300 mt-4">
                    ...
                    <div className="popup-content-empty" style={{ textAlign: "center" }}>
                        ...
                        <button onClick={handleSubmit} disabled={isSaving} className="bg-persian-blue-500 text-white py-2 px-4 rounded-md text-sm font-semibold hover:bg-persian-blue-600 transition duration-300" >
